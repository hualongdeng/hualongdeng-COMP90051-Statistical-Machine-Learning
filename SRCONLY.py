...
reg_result = 0
reg_result = get_lr_mse(np.append(male_train_x, female_train_x, axis=0), mix_dev_x, mix_test_x, np.append(male_train_y, female_train_y, axis=0), mix_dev_y, mix_test_y)
reg_result = reg_result + get_lr_mse(np.append(mix_train_x, female_train_x, axis=0), male_dev_x, male_test_x, np.append(mix_train_y, female_train_y, axis=0), male_dev_y, male_test_y)
reg_result = reg_result + get_lr_mse(np.append(male_train_x, mix_train_x, axis=0), female_dev_x, female_test_x, np.append(male_train_y, mix_train_y, axis=0), female_dev_y, female_test_y)
print(reg_result/3)

# calculate Neural Network result
net_result = 0
net_result = get_mse_net(np.append(male_train_x, female_train_x, axis=0), mix_dev_x, mix_test_x, np.append(male_train_y, female_train_y, axis=0), mix_dev_y, mix_test_y)
net_result = net_result + get_mse_net(np.append(mix_train_x, female_train_x, axis=0), male_dev_x, male_test_x, np.append(mix_train_y, female_train_y, axis=0), male_dev_y, male_test_y)
net_result = net_result + get_mse_net(np.append(male_train_x, mix_train_x, axis=0), female_dev_x, female_test_x, np.append(male_train_y, mix_train_y, axis=0), female_dev_y, female_test_y)
print(net_result/3)