...
_, male_train_x, _, male_train_y = train_test_split(male_train_x, male_train_y, test_size=1000, random_state=0)
_, female_train_x, _, female_train_y = train_test_split(female_train_x, female_train_y, test_size=1000, random_state=0)
_, mix_train_x, _, mix_train_y = train_test_split(mix_train_x, mix_train_y, test_size=1000, random_state=0)

# calculate LogisticRegression result
reg_result = 0
reg_result = get_lr_mse(mix_train_x, mix_dev_x, mix_test_x, mix_train_y, mix_dev_y, mix_test_y)
reg_result = reg_result + get_lr_mse(male_train_x, male_dev_x, male_test_x, male_train_y, male_dev_y, male_test_y)
reg_result = reg_result + get_lr_mse(female_train_x, female_dev_x, female_test_x, female_train_y, female_dev_y, female_test_y)
print(reg_result/3)

# calculate Neural Network result
net_result = 0
net_result = get_mse_net(mix_train_x, mix_dev_x, mix_test_x, mix_train_y, mix_dev_y, mix_test_y)
net_result = net_result + get_mse_net(male_train_x, male_dev_x, male_test_x, male_train_y, male_dev_y, male_test_y)
net_result = net_result + get_mse_net(female_train_x, female_dev_x, female_test_x, female_train_y, female_dev_y, female_test_y)
print(net_result/3)